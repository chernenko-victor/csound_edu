...
instr part_2bar_solo
 ...
 iProximityOffset = p4
	...
	if kTrig == 1 then
	 ...
	 kCurrEl Markovk gkSFC2, kCurrTickStress
	 	kStep	random 	.0, 2.5
	 	kCurrElNormalized = kCurrEl
	 	if iProximityOffset > 0 then
	 		kRndOffset = linrnd_high_k(0,9)
	 		kCurrElNormalized = TableFolding(2, kCurrElNormalized+kRndOffset, 9)
	 	endif
	 	
		event  	"i", 	"harmonic_additive",	0, kDur, 2*gkFrqBase*(gkFrqTranform12Tone[gkSystemHarmony[kCurrElNormalized][kStep]-1]), .2
...
i "part_2bar_solo" 5 25 0
s

i "part_2bar" 0 30
;i "alt_stab_part" 20 80
i "part_2bar_solo" 5 25 1
s
